...
import { BorderRightRounded, Cancel, Done, DoneAllRounded } from '@mui/icons-material';
...
const AdminOrdersTabs = () => {
  ...
  const completeOrder = async (id, type = 'normal') => {
    try {
      await axios.patch(type === 'bulk' ? `/bulk-orders/${id}/complete` : `/orders/${id}/complete`);
      showSnackbar(`Order #${id} completed.`, 'success');
  ...
  const handleOpenDialog = async (items) => {
                ...
                <IconButton
                  ...
                  onClick={() => {
                    if(order.status === 'Placed') {
                        completeOrder(isBulk ? order.bulk_order_id : order.order_id, isBulk ? 'bulk' : 'normal')
                      }
                    }}
                >
                  <DoneAllRounded />
